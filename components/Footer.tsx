...
export default function Footer() {
  return (
    <footer className="bg-church-dark text-stone-400 text-sm">
      ...
      <div className="max-w-6xl mx-auto px-5 py-10 flex flex-col md:flex-row md:items-center gap-8 md:gap-0 md:justify-between border-b border-white/[0.06]">
        ...
        <nav className="flex flex-wrap gap-x-7 gap-y-2">
          {navLinks.map(({ href, label }) => (
            <Link key={href} href={href} className="text-stone-500 hover:text-stone-200 transition-colors text-xs tracking-wide uppercase font-medium">
              {label}
            </Link>
          ))}
        </nav>
      ...
      {/* ── Info row ── */}
      <div className="max-w-6xl mx-auto px-5 py-5 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 sm:justify-between text-xs text-stone-600">
